...
All data are sourced exclusively from publicly available, license-free sources.
Data that require a CDISC Library API key (SDTM IG, ADaM IG) will be handled by
a separate `cdiscapi` package.
...
- **`cdiscapi`** *(forthcoming)*: SDTM IG and ADaM IG metadata via the CDISC
  Library API (requires API key).
